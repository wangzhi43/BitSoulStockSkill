...
description: 给用户提供自定义交易策略的回测、自定义策略选股、查询股票实时信息(包括当前价格、涨跌幅、成交量、开盘价、最高价、最低价等)。触发场景：1、用户询问股票价格、股票行情、查询股票当前的价格、获取股票的实时信息等。2、用户描述了一段策略后，请求使用该策略进行股票筛选。3、用户描述了一段交易策略后，请求使用该策略进行回测。4、用户要求收集大V帖子、订阅博主内容、追踪某个博主的发言。5、用户要求查询个股舆情、追踪某只股票的讨论热度、查询某股票的相关讨论。6、其他人任何情况用户指定要用BitSoulStockSkill时，强制使用该skill进行任务执行。
---
...
* 如果用户意图是查询当前的token:
    * bitsoul_skill_tmp_strategy.py中的 {mode} 占位符替换为 Token_rw
* 如果用户意图是收集大V社交媒体发言或股票舆情:
    * bitsoul_skill_tmp_strategy.py中的 {mode} 占位符替换为 User_exec
    * 使用scripts/stock_api.py中的crawl_bloggers或crawl_stock接口
    * 执行后数据会保存在系统临时目录下的bitsoul_stock_info_tmp目录下，你需要对里面的内容进行分析，提取关键信息，根据用户的提问总结回答。发言中不要让用户感知到数据抓取的步骤，只汇报信息结果
* 所有任务执行完毕后，立刻结束回答。
